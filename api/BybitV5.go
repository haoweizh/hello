	...
	for {
		...
		if connTick != nil {
			if err := SendToConnections(model.Bybit, connTick.(map[*model.WSConn]bool), pingMsg); err != nil {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("tick conn maintain error %s %s", model.Bybit, err.Error()))
			}
		}
		for _, account := range accounts {
			...
			if connOrder != nil {
				if err := SendToConnection(model.Bybit, connOrder.(*model.WSConn), pingMsg); err != nil {
					errMsg += err.Error()
					success = false
					model.Log(``, model.LogLevelError, ``, model.SystemAPI, "-ws-bybit trade ws ping client error "+err.Error())
				}
			} else {
				success = false
			}
			...
			if connOrderUpdate != nil {
				if err := SendToConnection(model.Bybit, connOrderUpdate.(*model.WSConn), pingMsg); err != nil {
					errMsg += err.Error()
					success = false
					model.Log(``, model.LogLevelError, ``, model.SystemAPI, "ws-bybit order update ws ping client error "+err.Error())
				}
			} else {
				success = false
			}
			if !success {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, "fail to ping ws bybit "+errMsg)
...
func WsLogInBybit(account *model.Account, conn *model.WSConn) (success bool) {
	...
	if err := SendToConnection(model.Bybit, conn, loginBytes); err != nil {
		model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to login bybit trade ws: %s return %s`, account.Key, err.Error()))
	} else {
		success = true
	}
	return success
...
func WsOrderServeBybit(account *model.Account) {
	...
	if valueOrder == nil || valueOrderUpdate == nil {
		connOrder, errOrder := model.WsAccountClient(model.Bybit, account.Key, bybitTradeWsUrl, wsOrderHandlerBybit)
		if errOrder != nil {
			model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, "bybit can not create ws order "+errOrder.Error())
		...
		connOrderUpdate, errOrderUpdate := model.WsAccountClient(model.Bybit, account.Key, bybitStreamUrl+`/v5/private`, wsOrdUdtHandlerBybit)
		if errOrderUpdate != nil {
			model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, "bybit can not create ws order update"+errOrderUpdate.Error())
...
func getMarketsBybitSpot(marketInfos map[string]*model.MarketInfo) {
	...
	if spotResp == nil || spotResp.RetCode != 0 {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("get bybit spot market error, resp: %s, httpErr: %v, jsonErr: %v", httpResp, httpErr, spotJsonErr))
		return
	}
	for _, symbolInfo := range spotResp.Result.List {
		...
		if symbolInfo.PriceFilter.TickSize == "" {
			model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("币种：%s 价格步长为空 resp：%v", symbol, symbolInfo))
			continue
		}
	...
	for {
		...
		for _, perpInfo := range perpResp.Result.List {
			...
			if priceMin != marketInfo.PriceIncrement {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("最小价格和价格步长不一致 perp info：%v", perpInfo))
				continue
			}
			maxLeverage, _ := strconv.ParseFloat(perpInfo.LeverageFilter.MaxLeverage, 64)
			if maxLeverage < model.DefaultLeverage {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("最大杠杆小于%d perp info：%v", model.DefaultLeverage, perpInfo))
				continue
			}
...
func handleBookBybit(environment *model.Environment, bookWsResp *dtos.BybitBookWsResp, symbol string) {
	...
	if bookWsResp.Type == "snapshot" {
		if len(bookWsResp.Data.B) == 0 || len(bookWsResp.Data.A) == 0 {
			model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`bybit no book data %s`, bookWsResp.Data.S))
			return
		}
		...
		if !haveOld {
			model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("币种：%s bidask没有bidask 却收到delta ws", symbol))
			return
		}
		oldBid := old.Bids[0]
		oldAsk := old.Asks[0]
...
var spotBookWsHandler = func(market string, conn *model.WSConn, event []byte) {
	...
	if jsonErr != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, `fail to unmarshal bybit spot book ws data json `+jsonErr.Error())
		return
	}
...
var tickHandlerBybit = func(market string, conn *model.WSConn, event []byte) {
	...
	if jsonErr != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, `fail to unmarshal bybit perp tick ws data json `+jsonErr.Error())
		return
	}
...
var perpBookWsHandler = func(market string, conn *model.WSConn, event []byte) {
	...
	jsonErr := json.Unmarshal(event, bookWsResp)
	if jsonErr != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, `fail to unmarshal bybit perp book ws data json `+jsonErr.Error())
		return
	}
	if strings.Contains(bookWsResp.Topic, "orderbook") {
		if bookWsResp.Data.S == "" {
...
func WsTickServeBybit(market string) (socketMap map[*model.WSConn]bool, msgChans []chan struct{}, connectErr error) {
	...
	spotBookSockets, spotBookChannels, spotBookErr := model.WebSocketClient(model.Bybit, bybitStreamUrl+`/v5/public/spot`,
	...
	perpBookSockets, perpBookChannels, perpBookErr := model.WebSocketClient(market, bybitStreamUrl+`/v5/public/linear`,
	...
	perpTickConns, perpTickChans, perpTickErr := model.WebSocketClient(market, bybitStreamUrl+`/v5/public/linear`,
...
var subscribeHandlerBybit = func(market string, connection *model.WSConn, subscribes []interface{}) error {
	...
	if err = SendToConnection(model.Bybit, connection, subscribeMessage); err != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(" bybit can not subscribe %s %s", subscribeMessage, err.Error()))
	}
	model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, `bybit subscribed `+string(subscribeMessage))
	return err
}
...
func GetCoinBalanceBybit(key, secret, accountType string) (balances []*model.Balance) {
	...
	if balanceResp == nil || balanceResp.RetCode != 0 {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("fail to refresh spot balance bybit, resp: %s httpErr: %v, jsonErr: %v", httpResp, httpErr, jsonErr))
		time.Sleep(time.Minute)
		return GetCoinBalanceBybit(key, secret, accountType)
	} else {
		model.Log(key, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf("get spot balance bybit success, %s resp: %s ", key[:5], httpResp))
	}
...
func getPositionsBybit(key, secret string) (success bool, positions []*Position, posBalance float64) {
	...
	for {
		...
		if positionResp == nil || positionResp.RetCode != 0 {
			model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("fail to refresh perp position bybit, resp: %s httpErr: %v, jsonErr: %v", positionHttpResp, positionHttpErr, positionJsonErr))
			time.Sleep(time.Minute)
			return getPositionsBybit(key, secret)
		} else {
			model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("get perp position bybit success, resp: %s ", positionHttpResp))
		}
...
func WithdrawBybit(key, secret, coin, chain, address, amount string) bool {
	...
	if httpErr != nil {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, `fail to withdraw bybit`+httpErr.Error())
		return false
	} else {
		model.Log(key, model.LogLevelInfo, ``, model.SystemAPI, string(httpResp))
		return true
	}
}
...
func _(key, secret, coin, amount, fromType, toType string) bool {
	...
	if httpErr == nil {
		...
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, `fail to transfer `+httpErr.Error()+string(httpResp))
	}
	return false
}
...
func setBybitMarginLeverage(key, secret string) {
	...
	if httpErr != nil {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to setBybitMarginLeverage when post %s`, httpErr.Error()))
		return
	}
	jsonData, jsonErr := util.NewJSON(httpResp)
	if jsonErr != nil {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to setBybitMarginLeverage when NewJson %s`, jsonErr.Error()))
		return
	}
	if jsonData != nil {
		code, codeErr := jsonData.Get("retCode").Int()
		if code != 0 || codeErr != nil {
			model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("fail to set bybit margin leverage, resp: %s codeErr: %v", httpResp, codeErr))
		}
	}
...
func setSymbolLeverageBybit(account *model.Account, symbol string) (setSuc bool) {
	...
	if marketType == model.MarketTypePerp {
		...
		if httpErr != nil {
			model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to setBybitPerpLeverage when request %s %s`, symbol, httpErr.Error()))
			return false
		}
		jsonData, jsonErr := util.NewJSON(httpResp)
		if jsonErr != nil {
			model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to setBybitPerpLeverage when NewJson %s %s`, symbol, jsonErr.Error()))
			return false
		}
		if jsonData != nil {
			code, codeErr := jsonData.Get("retCode").Int()
			if code != 0 || codeErr != nil {
				model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("fail to set bybit perp leverage , resp: %s codeErr: %v", httpResp, codeErr))
				return false
			} else {
				return true
			}
...
func setBybitPerpLeverage(key, secret string) {
	...
	for symbol := range symbols {
		...
		if marketType == model.MarketTypePerp {
			...
			if httpErr != nil {
				model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to setBybitPerpLeverage when request %s %s`, symbol, httpErr.Error()))
				continue
			}
			jsonData, jsonErr := util.NewJSON(httpResp)
			if jsonErr != nil {
				model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to setBybitPerpLeverage when NewJson %s %s`, symbol, jsonErr.Error()))
				continue
			}
			if jsonData != nil {
				code, codeErr := jsonData.Get("retCode").Int()
				if code != 0 || codeErr != nil {
					model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("fail to set bybit perp leverage , resp: %s codeErr: %v", httpResp, codeErr))
				}
			}
			time.Sleep(time.Minute)
		}
...
func placeOrderBybit(account *model.Account, isWs bool, order *model.Order, orderParam string) {
	...
	if isWs && value != nil && value.(*model.WSConn).Conn != nil {
		...
		if err := SendToConnection(model.Bybit, value.(*model.WSConn), msg); err != nil {
			model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to place bybit ws order %s %s`, string(msg), err.Error()))
		}
	} else {
		...
		if bybitOrderResp == nil || bybitOrderResp.RetCode != 0 {
			...
			model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("fail to create bybit order request: %v resp: %s httpErr: %v, jsonErr: %v", param, httpResp, httpErr, jsonErr))
		} else {
...
func cancelOrdersBybit(key, secret, symbol string) (result bool) {
	...
	if httpErr != nil {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to do post when cancelOrdersBybit %s`, httpErr.Error()))
		return
	}
	jsonData, jsonErr := util.NewJSON(httpResp)
	if jsonErr != nil {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to NewJson when cancelOrdersBybit %s`, jsonErr.Error()))
		return
	}
	if jsonData != nil {
...
func getFundingRateBybit(symbol string) (fundingRate *model.FundingRate) {
	...
	if !success {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, "fail to get bybit perp funding rate, GetFromStandard: "+symbol)
		return
	}
...
func queryOrderBybit(key, secret, symbol, orderId string) *model.Order {
	...
	if orderResp == nil || orderResp.RetCode != 0 {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("get bybit order detail error, resp: %s, httpErr: %v, jsonErr: %v", httpResp, httpErr, jsonErr))
		return nil
	}

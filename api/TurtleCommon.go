...
func AdjustPosHolding(key, secret string, setting *model.Setting, data *model.TurtleData, tick *model.BidAsk) {
	...
	if posMap[setting.Symbol] != nil { //setting.Chance和pos.Holding相乘小于零代表方向相反，此时设置为0
		if float64(setting.Chance)*posMap[setting.Symbol].Holding <= 0 &&
			...
			model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`update turtle grid Amount %s %s %e to %e`,
...
func CheckActiveTrail(account *model.Account, setting *model.Setting, data *model.TurtleData, bidAsk *model.BidAsk) (trailed bool) {
	...
	if setting.Chance > 0 && data.LowActTrail*data.ActivationRate > 0 && bidAsk.Bids[0].Price > data.LowActTrail*data.ActivationRate {
		...
		for _, order := range trails {
			order.Function = model.Close
			data.OrderAdjust[order.OrderId] = order
			model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`success trail sell %s %s amt %f at %f ratio %f ordId %s`,
		...
		for _, order := range trails {
			order.Function = model.Close
			data.OrderAdjust[order.OrderId] = order
			model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`success trail buy %s %s amt %f at %f ratio %f ordId %s`,
...
func HandleOrders(key, secret, market, symbol string, settings []*model.Setting, turtleData []*model.TurtleData,
	...
	if (len(settings) != 2 && len(settings) != 1) || len(settings) != len(turtleData) {
		model.Log(``, model.LogLevelError, ``, model.SystemCarry, `wrong combine turtle parameter`)
		return false
	}
	if len(settings) == 1 {
	...
	for _, order := range turtle.OrderAdjust {
		if order != nil {
			if order.OrderType == model.OrderTypeTrailStop {
				model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`get trail from last turtle data %s %s %s amt %f at %f ordId %s %s`,
...
func GetTurtleData(account *model.Account, setting *model.Setting, removed bool) (data *model.TurtleData, dataValid bool) {
	...
	if lockValue == nil {
		lock = &sync.Mutex{}
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`create lock %s %s`, account.Key, `refreshDynamic`))
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`need to create turtle data %s %s %s %s %d`,
	...
	getOne, getAll := CalcTurtleData(account, data, candles, int(setting.Seconds), setting.Market, setting.Function, float64(setting.ChanceLimit), setting.AmountRate)
	if !getOne {
		model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`fail to getOne %s %s %d %d`, setting.Market, setting.Symbol, data.DaysFar, setting.Seconds))
		return nil, false
	} else if !getAll {
		model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`fail to getAll %s %s %d %d`, setting.Market, setting.Symbol, data.DaysFar, setting.Seconds))
		return nil, true
	}
	if lastHandled {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`set turtle data liquidated to setting %s %s %s %v`,
	...
	if data.Amount > 0 && data.N > 0 {
		...
		if marketInfo == nil {
			model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`fail to get marketInfo %s %s`, setting.Market, setting.Symbol))
			return nil, false
		}
		util.StoreSyncMap(&TurtleDataSet, data, setting.Function, setting.Market, setting.Symbol, nowStr)
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`set turtle %s %s %s %s Amount:%e N:%e %d:%e-%e %d:%e-%e %v`,
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`set data %s %s %s %f %f`, setting.Market, setting.Symbol, setting.Function, data.N, data.Amount))
		return data, true
	} else {
		return nil, false
	}
...
func getTurtleCandles(account *model.Account, market, symbol string, far, seconds int, currentPeriod time.Time) (candles []*model.Candle) {
	...
	if lastCandle2 == nil {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`base on last 2 candle %s %s far %d %d n-n %f %f nv-nv %f %f len %d volume len %d`,
...
func findCandle(candles []*model.Candle, begin time.Time) (resultCandle *model.Candle) {
	...
	for _, candle := range candles {
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`no found candle %s %s %s %s`,
	...
	for i, setting := range settings {
		...
		if orderLong != nil {
			...
			if orderLong != nil && (orderLong.Status == model.CarryStatusSuccess || orderLong.DealAmount*3 > orderLong.Amount*2) {
				...
				model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`order break long %s %s %s %d %e %e id %s usdApi %v`,

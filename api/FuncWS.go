...
func _(environment *model.Environment, market string, symbols map[string]bool) (
	...
	case model.BinanceSpot:
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, " create KLine ws chan for "+market)
...
func CreateWSTick(environment *model.Environment, market string) (
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, " create depth chan for "+market)
	...
	if err != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemCarry, market+` can not create depth server `+err.Error())
	}
...
func SendToConnection(market string, connection *model.WSConn, msg []byte) (err error) {
	...
	if connection == nil {
		model.Log(``, model.LogLevelError, ``, model.SystemNetwork, `fail to write to nil connection `+market)
		return
	}
	if err = connection.WriteMsg(msg); err != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemNetwork, `fail to write to connection `+market+string(msg)+err.Error())
	}
	return err
}
...
func SendToConnections(market string, connections map[*model.WSConn]bool, msg []byte) (err error) {
	...
	for connection := range connections {
		...
		if err != nil {
			model.Log(``, model.LogLevelError, ``, model.SystemNetwork, fmt.Sprintf(
...
func UpdateOrderDeal(market, orderId, status, msg string, dealAmount float64) {
	...
	if order != nil {
		...
		if dealAmount >= preDeal {
			...
			model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`update deal at %d %s %s %s %f to %f %s`,
				i, order.Market, order.Symbol, order.OrderSide, preDeal, order.DealAmount, order.Status))
		}
	} else {
		model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`no order stored %s %s %s`, market, orderId, msg))
	}
}

...
func SetSetting(function, market, symbol string, setting *model.Setting) {
	settings := GetSettings(function, market)
	if settings == nil {
		settings = &sync.Map{}
		util.StoreSyncMap(symbolSettings, settings, function, market)
	}
	settings.Store(symbol, setting)
}

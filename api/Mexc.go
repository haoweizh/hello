...
func WsDepthServeMexc(markets *model.Markets, orderHandler OrderHandler, useFullDepthSub bool) (channels []chan struct{}, err error) {
	...
	if !useFullDepthSub {
		...
		contractIncreSubs := GetWSSubscribes(model.Mexc, mexcContractDepthIncreSubType)
...
func contractDepthIncreWsHandlerMexc(markets *model.Markets, resp *dtos.MexcContractDepthWsResp, orderHandler OrderHandler) {
	if resp == nil {
		return
	}
...
func contractTickerWsHandlerMexc(markets *model.Markets, msg []byte, orderHandler OrderHandler) {
...
func getSubTypeFromChannelKey(chanKey string) string {
	ret := strings.Split(chanKey, "-")
	if len(ret) > 0 {
		return ret[0]
	}

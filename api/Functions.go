...
func RequireDepthChanReset(markets *model.Markets, market string) bool {
	...
	for symbol := range symbols {
		...
		if delay < model.AppConfig.Delay {
			...
			util.Notice(fmt.Sprintf(`RequireDepthChanReset delay too long %s %s %f`, market, symbol, delay))
		}
	}
...
func GetFundingRate(key, secret, market, symbol string) (success bool, rate float64, updateTime time.Time) {
	...
	if fundingRate != nil && market == model.OKEX {
		if now < fundingRate.ExpireTime-60 && fundingRate.UpdateTime.Add(time.Minute*5).After(time.Now()) {

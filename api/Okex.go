...
var wsAccountHandlerOKEX = func(market, key string, event []byte) {
	...
	if responseJson.Get(`event`).MustString() == `login` && responseJson.Get(`code`).MustString() == `0` {
		...
		// 订阅私有频道
		subscribePrivateOKEX(value.(*model.WSConn), connKey)
	}
...
func subscribePrivateOKEX(conn *model.WSConn, connKey string) {
	...
	err = conn.WriteJson(map[string]interface{}{"op": "subscribe", "args": []interface{}{map[string]string{"channel": "orders", "instType": "SWAP"}}})
	if err != nil {
		util.Log(util.LogLevelError, fmt.Sprintf(`fail to sub %s swap order update`, model.OKEX))
		model.AppEnvironment.ConnOrder.Delete(connKey)
		return
	}
...
func setSymbolLeverageOkx(account *model.Account, symbol string) (setSuc bool) {
	...
	response, _ := sendSignRequestOKEX(account, http.MethodPost, `/api/v5/account/set-leverage`,
		nil, map[string]interface{}{`instId`: symbol, `mgnMode`: `cross`, `ccy`: `USDT`, `lever`: strconv.Itoa(leverage)})
	responseJson, err := util.NewJSON(response)
	if err != nil || responseJson.Get(`code`).MustString() != `0` {
		return false
	}

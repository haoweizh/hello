	...
	for {
		...
		if connTick != nil {
			if err := SendToConnections(model.OKEX, connTick.(map[*model.WSConn]bool), []byte(`ping`)); err != nil {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("tick conn maintain error %s %s", model.OKEX, err.Error()))
			}
		}
		for _, account := range accounts {
			...
			if value != nil {
				if err := SendToConnection(model.OKEX, value.(*model.WSConn), []byte(`ping`)); err != nil {
					model.Log(``, model.LogLevelError, ``, model.SystemAPI, "-test ok ws-okex server ping client error "+err.Error())
					success = false
				}
			} else {
				success = false
			...
			if !success {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`-test ok ws- no private connection %s`, account.Key))
...
func reSubscribe(subscribes []interface{}) {
	...
	for _, item := range subscribes {
		...
		if !success || bidAsk == nil || time.Now().UnixMilli()-int64(bidAsk.Ts) > 60000000 {
			if bidAsk == nil {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, `bid ask nil okex `+symbol)
			} else {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`bid ask too late okex %s %d %d`, symbol, time.Now().UnixMilli(), bidAsk.Ts))
			}
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`no need reset %s`, model.OKEX))
	if len(subArray) == 0 {
		return
	}
	...
	if err := SendToConnections(model.OKEX, connValue.(map[*model.WSConn]bool), util.JsonEncodeToByte(subscribeMap)); err != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, "okex can not unsubscribe "+err.Error())
	}
	...
	for i, item := range subArray {
		...
		if reSubErr := SendToConnection(model.OKEX, connection, util.JsonEncodeToByte(subscribe)); reSubErr != nil {
			model.Log(``, model.LogLevelError, ``, model.SystemAPI, "okex can not re-subscribe "+reSubErr.Error())
		} else {
			model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`okex resubscribe %s`, item[`instId`]))
		}
	}
}
...
var subscribeHandlerOKEX = func(market string, connection *model.WSConn, subscribes []interface{}) error {
	...
	if err = SendToConnection(model.OKEX, connection, subscribeMessage); err != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, "okex can not subscribe "+err.Error())
		return err
	}
	return err
}
...
var wsAccountHandlerOKEX = func(market, key string, event []byte) {
	...
	if responseJson.Get(`event`).MustString() == `login` && responseJson.Get(`code`).MustString() == `0` {
		...
		if err != nil {
			model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to sub %s spot order update`, market))
		...
		if err != nil {
			model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to sub %s swap order update`, market))
...
func wsLogInOKEX(account *model.Account, conn *model.WSConn) (success bool) {
	...
	if err := SendToConnection(model.OKEX, conn, util.JsonEncodeToByte(loginMap)); err != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to login okex ws: %s return %s`, account.Key, err.Error()))
	} else {
		success = true
		model.Log(``, model.SystemAPI, ``, model.SystemAPI, fmt.Sprintf(`login okex ws: %s`, account.Key))
	}
	return
}
...
func WsOrderServeOKEX(account *model.Account) {
	...
	if err != nil {
		model.Log(``, model.LogLevelError, ``, model.SystemAPI, "can not create web socket "+err.Error())
	} else if conn != nil {
	...
	if data[`checksum`] != nil {
		...
		if ok && value.(bool) {
			...
			if !success && time.Now().Minute() == 0 && time.Now().Second() == 0 {
				model.Log(``, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf("%v ts %d checksum %s %s \n %v",
...
func sendSignRequestOKEX(key, secret, method, path string, param, body map[string]interface{}) ([]byte, error) {
	...
	model.Log(``, model.LogLevelDebug, ``, model.SystemAPI, logMsg)
...
func PlacePairOKEX(account *model.Account, requestId, symbolBuy, symbolSell, orderType string, priceBuy, priceSell, amount float64) (success bool, errMsg string) {
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`place pair %s`, msg))
	if model.AppConfig.Env != `test` {
		...
		if err != nil {
			...
			model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`-test ok ws- success send ws order %s %s`, account.Key, msg))
		}
	}
	return true, ``
}
...
func placeOrderOKEX(account *model.Account, isWs bool, order *model.Order) {
	...
	if isWs {
		...
		model.Log(account.Key, model.LogLevelInfo, ``, model.SystemAPI, `ws order `+string(wsOrderMsg))
		order.Status = model.CarryStatusWorking
		value, _ := util.LoadSyncMap(&model.AppEnvironment.ConnOrder, model.OKEX, account.Key)
		if value == nil || value.(*model.WSConn).Conn == nil {
			model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`-test ok ws-fail to get private connection %s`, account.Key))
			order.Status = model.CarryStatusFail
		} else {
			if err := SendToConnection(model.OKEX, value.(*model.WSConn), wsOrderMsg); err != nil {
				model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`-test ok ws-fail to send order ws %s %s return %s`, account.Key, order.Symbol, err.Error()))
				order.Status = model.CarryStatusFail
			} else {
				model.Log(account.Key, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`-test ok ws- success place okex ws order %s %s`, account.Key, order.Symbol))
			}
		}
	} else {
		responseBody, httpErr := sendSignRequestOKEX(account.Key, account.Secret, http.MethodPost, path, nil, postData)
		model.Log(account.Key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`place okex %s return %s`, path, string(responseBody)))
		if httpErr != nil {
...
func cancelAllOkex(key, secret string) {
	...
	for _, order := range orders {
		...
		model.Log(key, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`cancelAllOkex %s id %s type %s return %v code %s %s`,
...
func cancelOrdersOKEX(key, secret, symbol string) (result bool, code, msg string) {
	...
	if len(normalOrders) > 20 || len(algoOrders) > 10 || len(advOrders) > 10 {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fatal error: too many normal order to be canceled normal: %d algo: %d adv:%d`,
...
func queryOpenOrdersOKEX(key, secret, symbol string, conditional bool) (orders []*model.Order) {
	...
	if err == nil {
		if `50011` == strings.Trim(orderJson.Get(`code`).MustString(), ` `) {
			model.Log(key, model.LogLevelError, ``, model.SystemAPI, `sleep 1 min and re-query orders when get code 50011`)
...
func getBalanceOKEX(key, secret string) (success bool, balances []*model.Balance, totalInUsd float64, collateral *Collateral) {
	...
	if err != nil || responseJson == nil || responseJson.GetPath(`data`) == nil || responseJson.Get(`code`).MustString() != `0` {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, `fail to get okex balance `)
		time.Sleep(time.Minute * 5)
...
func getPositionsOKEX(key, secret string) (success bool, positions []*Position) {
	...
	if err != nil || responseJson == nil || responseJson.Get(`code`).MustString() != `0` {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, `fail to get okex positions `)
		time.Sleep(time.Minute)
	...
	if arrayErr != nil {
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to get okex positions %s`, arrayErr.Error()))
		time.Sleep(time.Minute)
		return getPositionsOKEX(key, secret)
	}
	...
	if data[`maxSell`] != nil {
		...
		if marketType == model.MarketTypeSpot {
			...
			if ok {
				...
				model.LogLess(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to get price from ok bidAsk %s`, symbol))
			}
		}
	}
...
func getFundingRateOKEX(key, secret, symbol string) (fundingRate *model.FundingRate) {
	...
	if fundingJson == nil || fundingJson.Get(`data`) == nil || fundingJson.Get(`data`).MustArray() == nil ||
		...
		model.Log(key, model.LogLevelError, ``, model.SystemAPI, fmt.Sprintf(`fail to getFundingRateOKEX %v`, fundingErr))
		return nil
	}
...
func getCandlesOKEX(account *model.Account, symbol string, before, after time.Time, count, slotSeconds int) (
	...
	if model.AppRedis != nil {
		...
		if redisErr == nil {
			...
			isCache = true
			model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`get candles from key %s %d`, redisKey, len(temp)))
		}
	}
	if responseBody == nil {
		isCache = false
	...
	if err != nil || candleJson == nil || candleJson.Get(`data`) == nil || len(candleJson.Get(`data`).MustArray()) == 0 {
		if model.AppRedis != nil {
			...
			model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`del redis key %s`, redisKey))
		}
		return
	} else if !isCache && model.AppRedis != nil {
		model.Log(``, model.LogLevelInfo, ``, model.SystemAPI, fmt.Sprintf(`set candles to cache %s len %d`, redisKey, len(string(responseBody))))

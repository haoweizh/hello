...
//var huobiPositionMap = make(map[string]*api.Position)
//
...
//		value, _ := model.AppEnvironment.ConnTick.Load(model.HuobiSpot)
//		if value == nil {
//			return
//		}
...
//				funcHandlers := api.GetFunctions(model.HuobiSpot, symbol)
//				if funcHandlers != nil {
//					funcHandlers.Range(func(function, value interface{}) bool {
//						setting := api.GetSetting(function.(string), model.HuobiSpot, symbol)
...
//			funcHandlers := api.GetFunctions(model.HuobiPerp, symbol)
//			if funcHandlers != nil {
//				funcHandlers.Range(func(function, value interface{}) bool {
//					setting := api.GetSetting(function.(string), model.HuobiPerp, symbol)
//					if setting != nil && value != nil && value.(model.CarryHandler) != nil {
//						go value.(model.CarryHandler)(setting, &bidAsk)
//					}
//					return true
...
//	subscribes := api.GetWSSubscribes(model.HuobiSpot, []string{model.SubscribeTicker})
...
////}
//
//func getPositionsHuobiPerp(key string, secret string) (success bool, positions []*api.Position, accountValue, availableU float64) {
//	postData := make(map[string]interface{})
...
//	positionMap := make(map[string]*api.Position)
...
//			positionMap[currency] = &api.Position{Market: model.HuobiSpot, Ts: util.GetNowUnixMillion(),

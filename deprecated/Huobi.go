...
//			if err := api.SendToAllConnections(model.Huobi, pingParams); err != nil {
...
//						Market: model.Huobi, Symbol: symbol, Side: model.OrderSideBuy}},
//					Asks: []model.Tick{{Price: tickJson.Get("ask").MustFloat64(), Amount: tickJson.Get("askSize").MustFloat64(),
//						Market: model.Huobi, Symbol: symbol, Side: model.OrderSideSell}}}
...
//			pingMap["pong"] = responseJson.Get(`ping`).MustInt()
//			pingParams := util.JsonEncodeToByte(pingMap)
//			if wsErr := api.SendToAllConnections(model.Huobi, pingParams); wsErr != nil {
...
//				return
//			}
//			bidAsk.Bids = []model.Tick{{Price: bidPrice, Amount: bidAmount, Market: model.Huobi, Symbol: symbol, Side: model.OrderSideBuy}}
//			bidAsk.Asks = []model.Tick{{Price: askPrice, Amount: askAmount, Market: model.Huobi, Symbol: symbol, Side: model.OrderSideSell}}
//
//			haveOld, old := markets.GetBidAsk(symbol, model.Huobi)
//			if haveOld && old.UpdateId > bidAsk.UpdateId {
//				return
...
//	subscribes := api.GetWSSubscribes(model.Huobi, model.SubscribeTicker)
...
//		}
//	}
//	channels, channelErr := api.WebSocketClient(model.Huobi, wsHuobi, spotSubscribes, subscribeHandlerHuobi, wsHandler,
//		orderHandler, wsStepHuobi)
//	if channelErr != nil {
//		util.SocketInfo(`fail to create huobi conn %s`, channelErr.Error())
//	}
//	dmChannels, dmChannelErr := api.WebSocketClient(model.Huobi, wsHuobiFuture, futureSubscribes, subscribeHandlerHuobi,
...
//}
//
//func getPositionsHuobi(key string, secret string) (success bool, positions []*model.Position, accountValue, availableU float64) {
//	postData := make(map[string]interface{})

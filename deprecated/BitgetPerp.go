...
//		fundingResp, _ := util.HttpRequest(http.MethodGet, api.bitgetRestUrl+
...
//	httpResp, httpErr := util.HttpRequest(http.MethodGet, api.bitgetRestUrl+"/api/v2/mix/market/contracts?productType=USDT-FUTURES", "", map[string]string{}, 30)
...
//	client := dtos.BitgetRestClient{BaseUrl: api.bitgetRestUrl, Passphrase: model.AppConfig.Phase, ApiKey: account.Key, ApiSecretKey: account.Secret}
...
//	symbols := api.GetMarketSymbols(model.BitgetPerp)
...
//	client := dtos.BitgetRestClient{BaseUrl: api.bitgetRestUrl, Passphrase: model.AppConfig.Phase, ApiKey: key, ApiSecretKey: secret}
...
//			}
//			api.SetFundingRate(model.BitgetPerp, symbol, fundingRate)
//		}
//	}
//}
//
...
//	depthSubs := api.GetWSSubscribes(market, []string{model.SubscribeDepth})
//	marketPriceSubs := api.GetWSSubscribes(market, []string{model.SubscribeMarkPrice})
//	markPriceSockets, markPriceErr := model.WsPublicClient(market, api.bitgetPublic,
//		marketPriceSubs, api.subscribeHandlerBitget, markPriceWsHandler, wsStepBitget)
//	if markPriceErr == nil {
...
//	}
//	perpBookSockets, perpBookErr := model.WsPublicClient(market, api.bitgetPublic,
//		depthSubs, api.subscribeHandlerBitget, api.tickHandlerBitget, wsStepBitget)
//	if perpBookErr == nil {
...
//		api.bitgetRestUrl, path, dialectSymbol, `USDT-FUTURES`), ``, nil, 30)
...
//		connKey := api.getPrivateConnKey(model.BitgetPerp, account.Key, model.MarketTypePerp)
...
//			api.HandleWsOrderConnFail(account, model.BitgetPerp, order)
//		}
//	} else {
//		client := dtos.BitgetRestClient{BaseUrl: api.bitgetRestUrl, Passphrase: model.AppConfig.Phase, ApiKey: account.Key, ApiSecretKey: account.Secret}

...
func Test_initTurtleN(t *testing.T) {
	...
	model.NewConfig()
	_ = configor.Load(model.AppConfig, "./config.yml")
	carryClose, rate := model.AppConfig.GetCarrySetting(model.Binance, 3)
	fmt.Println(fmt.Sprintf(`%v %v`, carryClose, rate))
	carryClose, rate = model.AppConfig.GetCarrySetting(model.Binance, 0)
	fmt.Println(fmt.Sprintf(`%v %v`, carryClose, rate))
	carryClose, rate = model.AppConfig.GetCarrySetting(model.Binance, 1)
	fmt.Println(fmt.Sprintf(`%v %v`, carryClose, rate))
	carryClose, rate = model.AppConfig.GetCarrySetting(model.OKEX, 0)
	fmt.Println(fmt.Sprintf(`%v %v`, carryClose, rate))
	carryClose, rate = model.AppConfig.GetCarrySetting(model.Gate, 1)
	fmt.Println(fmt.Sprintf(`%v %v`, carryClose, rate))
	carryClose, rate = model.AppConfig.GetCarrySetting(model.Ftx, 2)
	fmt.Println(fmt.Sprintf(`%v %v`, carryClose, rate))

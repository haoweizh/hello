...
var ProcessTurtle = func(setting *model.Setting, tick *model.BidAsk) {
	...
	if setting == nil || tick == nil || tick.Asks == nil || tick.Bids == nil || model.AppConfig.Handle != `1` ||
		(ok && maintaining.(bool)) || (model.AppConfig.Env != `test` && now-int64(tick.Ts) > 10000) || time.Now().Minute() == 0 {
		return
	}
	...
	data, setting = api.GetTurtleData(account.Key, account.Secret, setting.Function, setting.Symbol, setting, true)
	if data == nil || data.N == 0 || data.Amount == 0 || setting == nil {
	...
	msgKey := util.GetMsgKey(setting.Function, setting.Market, setting.Symbol)

...
var ProcessTurtle = func(setting *model.Setting, tick *model.BidAsk) {
	...
	msg := fmt.Sprintf("[%d:%d %d:%d]%s N-VolumeQuote %f 可开%v 当前已经持仓数量:%e 持仓数/限制:%d/%d "+
	...
	if setting.Chance == 0 { // 开初始仓
		...
		if data.UseNear {
			...
			priceShort = math.Max(data.HighFar, data.HighToday) - priceChange
		}
		placeTurtleOrders(account.Key, account.Secret, data, setting, canOpenTurtle, chanceInAll, priceShort, priceLong, tick)
		// 加仓一个单位
		if data.BreakLong && data.OrderLong != nil {
		...
		if data.UseNear {
			...
			if data.LowToday > 0 {
				priceLong = math.Min(data.LowFar, data.LowToday) + priceChange
			} else {

...
var ProcessCombineTurtle = func(settingCombine *model.Setting, tick *model.BidAsk) {
	...
	if settingNormal == nil {
		model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`combine return no normal setting from %s %s`, market, symbol))
		return
	}
	account := model.AppConfig.GetAccounts(market)[0]
	if (settingCombine.Chance != 0 && settingCombine.PriceX == 0) || (settingNormal.Chance != 0 && settingNormal.PriceX == 0) {
		if time.Now().Second() == 0 {
			model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`combine return no last priceX %s %s %d %e %d %e`,
	...
	if dataCombine == nil || dataNormal == nil || settingCombine == nil || settingNormal == nil ||
		...
		if !removed && (time.Now().Minute() == 0 && time.Now().Second() == 0) {
			model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`combine return no turtle combine turtle %s %s`, market, symbol))
		}
		return
	}
	if time.Now().After(dataCombine.Expire) || time.Now().After(dataNormal.Expire) {
		model.LogLess(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`turtle data expired %s %s`, settingCombine.Market, settingCombine.Symbol))
		return
	}
	if !dataCombine.OrderCleared {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`combine return not cleared %s %s %v`, market, symbol, dataCombine.OrderCleared))
		return
	}
	if dataNormal.N == 0 || dataNormal.Amount == 0 || dataCombine.N == 0 || dataCombine.Amount == 0 {
		model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`invalid turtle data n %s %s %f %f %f %f`,
...
func handleBreak(setting *model.Setting, data *model.TurtleData, orders []*model.Order, orderBreak bool) (work bool) {
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`query %s break %s %s %s %d %s %s chances %d`,
	...
	if orders[0].RefreshType != model.FunctionTurtleAdjust && orders[0].Function == model.Close {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`加%s %s %s chance:%d Amount:%e px:%e order type %s`,
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`clear turtle buy when sell break %s %s %v`, setting.Market, setting.Symbol, orders))
	return true
}
...
func placeTurtleLong(account *model.Account, orderType string, data *model.TurtleData, setting *model.Setting,
	...
	if setting.Chance < 0 {
		...
		if amount == 0 {
			model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`fail to place short amt 0 from grid amt, set chance 0 %s %s %d`,

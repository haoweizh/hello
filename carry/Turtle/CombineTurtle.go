...
var ProcessCombineTurtle = func(settingCombine *model.Setting, tick *model.BidAsk) {
	...
	dataCombine, settingCombine = api.GetTurtleData(account.Key, account.Secret, model.FunctionCombineTurtle,
		settingCombine.Symbol, settingCombine, true)
	dataNormal, settingNormal = api.GetTurtleData(account.Key, account.Secret, model.FunctionTurtleNormal,
		settingNormal.Symbol, settingNormal, true)
	if dataCombine == nil || dataCombine.N == 0 || dataCombine.Amount == 0 || dataNormal == nil ||
		dataNormal.N == 0 || dataNormal.Amount == 0 || settingCombine == nil || settingNormal == nil {
	...
	msgKey := util.GetMsgKey(model.FunctionCombineTurtle, market, symbol)
	msg := fmt.Sprintf("[%d-%d %d:%d]%s N-Volume %f 可开%vbig:%d 币种数:%d/%d %d日:%e-%e %d日:%e-%e N:%e 仓数上限%d "+
		...
		dataCombine.HighDaysNear, dataCombine.N, int(settingCombine.OpenShortMargin), dataCombine.Amount,
		tick.Bids[0].Price, tick.Asks[0].Price, settingNormal.Chance, settingNormal.GridAmount, settingNormal.PriceX,
		dataNormal.Liquidated, dataNormal.GetIds(), settingCombine.Chance, settingCombine.GridAmount,

...
func createFromBalance(account *model.Account, setting *model.Setting) (carryStatus *model.CarryStatus) {
	...
	if carryStatus.DoRevert {
		util.Log(util.LogLevelError, fmt.Sprintf(`do revert true %d %s %s value big balance u %f<%f || %f>0.2 %f/%f`,
			account.Index, setting.Market, setting.Symbol, sm.availableU, usdLowLine, carryStatus.RateInAll, sm.totalLong, sm.accountValueInU))
	}
	return carryStatus
}

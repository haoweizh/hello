...
func createFromPosition(account *model.Account, setting *model.Setting) (carryStatus *model.CarryStatus) {
	...
	if value == nil || !ok {
		cm := createContractMarket(account, setting.Market)
	...
	_, price := api.GetPriceForce(setting.Market, setting.Symbol, true)
	limitAmount := 0.0
	availableAmount := 0.0
...
func createFromBalance(account *model.Account, setting *model.Setting) (carryStatus *model.CarryStatus) {
	...
	if sm.balances[setting.Symbol] != nil {
		...
		limitSell := math.Max(balance.AvailableWithBorrow, 0)
	...
	if carryStatus.DoRevert {
		util.Log(util.LogLevelLocal, fmt.Sprintf(`do revert true %d %s %s value big balance u %f<%f || %f>0.2 %f/%f holding %f`,
			account.Index, setting.Market, setting.Symbol, sm.availableU, usdLowLine, carryStatus.RateInAll, sm.totalLong, sm.accountValueInU, carryStatus.Holding))
	}
	return carryStatus
}
...
func initStatus(account *model.Account, setting *model.Setting) (status *model.CarryStatus) {
	...
	if setting.Market == model.BinanceSpot {
		status.DoRevert = true
	}
...
func FailOrdersReconnect() {
	...
	for index, marketMap := range failOrders {
		...
		for market, order := range marketMap {
			specialChan := api.GetSpecialChan(market)
			// special chan无需重连
			if specialChan == 1 && accounts[market].Index == 0 {
...
func resetAccounts(doEqual bool, unixSeconds int64) {
	...
	for !util.Terminal {
		...
		if finish {
			util.Log(util.LogLevelInfo, fmt.Sprintf(`exit clearing cross accounts %#v`, waitReset))
			break
		}
	}
	for i := 0; i < api.GetAccountsLen(); i++ {
	...
	for i := 0; i < api.GetAccountsLen(); i++ {
		...
		for _, market := range model.AppEnvironment.Markets {
			if doEqual {
				...
				if market == model.Gate && account != nil {
...
func placeEqual(status *model.CarryStatus, price, amount float64, orderSide string) (dealAmount float64) {
	...
	if checkAmount > 0 {
		...
		order := api.PlaceOrder(status.Account, orderSide, model.OrderTypeLimit, status.Market, status.Symbol, status.Setting.Coin,
			orderParam, model.FunctionCompAll, ``, price, price, amount, false, nil)
...
var ProcessCross = func(setting *model.Setting, tick *model.BidAsk) {
	...
	if tick == nil || tick.Asks == nil || tick.Bids == nil || setting == nil || setting.Valid == false ||
		model.AppEnvironment.CrossEqualing || model.AppEnvironment.CrossPause || model.AppEnvironment.CrossStop ||
...
func placeCross(carryCoin *model.CarryCoin, statusBuy, statusSell *model.CarryStatus, priceBuy, priceSell,
	...
	go api.PlaceOrder(statusBuy.Account, model.OrderSideBuy, model.OrderTypeLimit, statusBuy.Market, statusBuy.Symbol,
		statusBuy.Setting.Coin, orderParamBuy, model.FunctionCross, closeType, priceBuy, priceBuy, amountBuy, true, PostOrderCross)
	go api.PlaceOrder(statusSell.Account, model.OrderSideSell, model.OrderTypeLimit, statusSell.Market, statusSell.Symbol,
		statusSell.Setting.Coin, orderParamSell, model.FunctionCross, closeType, priceSell, priceSell, amountSell, true, PostOrderCross)
...
var PostOrderCross = func(order *model.Order) {
	...
	if !order.HaveId() || order.ErrCode != `` || order.Status == model.CarryStatusFail {
		...
		if value != nil {
			...
			util.Log(util.LogLevelLocal, fmt.Sprintf(`stop trade %s %s %s %d %s %s %s`,

...
func _() {
	for true {
		...
		for _, market := range markets {
			...
			_, balance, _, _ := api.GetBalances(``, ``, market)
	...
	for true {
		...
		for true {
			...
			model.AppDB.Limit(100).Offset(feeIndex).Where(
				`created_at>? and created_at<? and status=? and refresh_type!=? and refresh_type!=?`,
				lastDays2, lastMin10, model.CarryStatusWorking, model.FunctionDCarry, model.FunctionCarry).Find(&orders)

	...
	for true {
		...
		for true {
			...
			model.AppDB.Limit(100).Offset(feeIndex).Where(
				`date(order_time)>? and status=? and refresh_type!=? and refresh_type!=?`,
				lastDays2, model.CarryStatusWorking, model.FunctionComplement, model.FunctionCarry).Find(&orders)
			if len(orders) == 0 {
				break
			}

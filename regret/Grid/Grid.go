...
func createOrders(setting *model.Setting, data *Data, candle *model.Candle) {
	...
	if data.orderBuy == nil && setting.Chance < 3 && (canOpen || setting.Chance < 0) {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`create order %s %s %s amt %f at %f %s`,
...
var ProcessGrid = func(start, end time.Time, setting *model.Setting, candles model.Candles) {
	...
	if gridCandles != nil && gridCandles.Len() > 0 && gridCandles[0] != nil && gridCandles[len(gridCandles)-1] != nil {
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`get sorted gridCandles from %s %s to %s %s`,
	...
	for i := 0; i < len(candles); i++ {
		if candles[i] == nil {
			model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`fail to get market candle %s %s %d`, setting.Market, setting.Symbol, i))
			continue
		}
		...
		if gridMap[sign] == nil {
			if msgMiss != fmt.Sprintf(`fail to get combine candle %s`, sign) {
				...
				model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, msgMiss)
			}
			continue
		}
		if msgHandle != fmt.Sprintf(`deal grid candle %s`, sign) {
			msgHandle = fmt.Sprintf(`deal grid candle %s`, sign)
			model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, msgHandle)
		}
...
func handleGrid(setting *model.Setting, data *Data, candle *model.Candle) {
	...
	if data.orderSell != nil && data.orderSell.Status == model.CarryStatusWorking && candle.PriceHigh > data.orderSell.Price {
		...
		data.orderBuy = nil
		data.orderSell = nil
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`set setting %s %s chance %d amt %f time %s`,

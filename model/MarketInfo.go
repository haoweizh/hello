...
var CommonSymbols = map[string]bool{`btc_perp`: true, `BTC_PERP`: true, `ETH_PERP`: true, `eth_perp`: true, `SOL_PERP`: true, `sol_perp`: true}
...
func ParseRealAmount(market, symbol string, amount float64) (success bool, realAmount float64) {
	v, _ := util.LoadSyncMap(MarketInfos, market, symbol)
	if v == nil {
		util.LogLess(util.LogLevelError, fmt.Sprintf(`ParseRealAmount fail no marketinfo %s %s`, market, symbol))
		return false, 0
	}

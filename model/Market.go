...
type Environment struct {
	...
	AggregateCandles *sync.Map // sync.Map[market]*sync.Map[symbol]*sync.Map[interval] *AggregateCandles
	WsManager        *WSManager
}
...
func (environment *Environment) SetCandle(symbol, market string, candle *Candle) bool {
	...
	if last == nil || last.(*Candle).Begin.Before(candle.Begin) {
		//fmt.Println(fmt.Sprintf(`set candle to %s %s %s`, symbol, market, candle.Begin.String()))

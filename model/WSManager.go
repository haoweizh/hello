...
func (manager *WSManager) WrapSend(address string) {
	defer func() {
		if r := recover(); r != nil {
			Log(``, LogLevelError, ``, SystemNetwork, `recovered from panic`)
		}
	}()
...
func (manager *WSManager) Update(address string, aggregateCandle *AggregateCandle) (duplicated bool) {
	...
	if !duplicated {
		Log(``, LogLevelInfo, ``, SystemCarry, fmt.Sprintf(`update new aggregate %s %f %f %f`,
...
func (manager *WSManager) RemoveAgent(address string, wsAgent *WSAgent) {
	...
	Log(``, LogLevelInfo, ``, SystemCarry, fmt.Sprintf(`remove agent %s %v`, address, wsAgent))
}
...
func (manager *WSManager) AddAgent(wsAgent *WSAgent) {
	...
	agents.(*sync.Map).Range(func(key, value any) bool {
		Log(``, LogLevelInfo, ``, SystemCarry, fmt.Sprintf(`got agent %s %v`, wsAgent.Address, key))
		return true
	})
}
	...
	for {
		...
		case message, ok := <-agent.ChanWrite:
			...
			if err != nil {
				...
				Log(``, LogLevelError, ``, SystemNetwork, fmt.Sprintf(`fail to send ws update %v %s`, agent, err.Error()))
			}
		}
	}
}
	...
	for {
		...
		case <-time.After(20 * time.Second):
			if agent.Pinged {
				...
				Log(``, LogLevelError, ``, SystemNetwork, `time out without ping`)
				return
			}
		}
	}

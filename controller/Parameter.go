...
func autoSimulate(market, coins string, begin, end time.Time, strBegin, strEnd string, useNear, useM bool,
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`del %s %d rows affected`, sign, delNum))
	regret.ProcessCandles(begin, end, far, allLimit, useNear, useM, market, sign, settings)
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`auto simulation done %s`, sign))
	...
	for _, coin := range coinArr {
		if strNew == `true` {
			...
			for _, setting := range settings {
				...
				model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`del rows %d %s %s %s %s~%s`, delNum, setting.Function, market, setting.Symbol, strBegin, strEnd))
				Grid.ProcessGrid(begin, end, setting, candles)
			}
		}
	}
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`done simulate grid %s %v`, market, coins))
...
func simulateGXZQ(c *gin.Context) {
	...
	for i := 3; i <= 21; i++ {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`done cut tail %s %s %s`, market, coins, sign))
		sign = fmt.Sprintf(`market%s,coins%s,seconds86400,%s~%s,far%d,near%d,limit%d,allLimit%d,useNear%v`,
			market, coins, strBegin, strEnd, i*2, i, 3, -1, false)
		regret.CutTail(market, coins, sign)
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`done cut tail %s %s %s`, market, coins, sign))
	}
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, `done cut tail all`)
	c.String(http.StatusOK, `done`)
}
...
func simulate(c *gin.Context) {
	...
	model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`get simulation parameter %s auto%s`, sign, auto))
	...
	if sessionValue != `haoweizh@qq.com` {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`all auto simulation done fee %f`, fee))
	...
	if strNew == `true` {
		...
		model.Log(``, model.LogLevelInfo, ``, model.SystemCarry, fmt.Sprintf(`no need process simulate new %s`, strNew))
	}
...
func holdPage(c *gin.Context) {
	...
	if carryRows != nil {
		...
		err := carryRows.Close()
		if err != nil {
			model.Log(``, model.LogLevelError, ``, model.SystemCarry, fmt.Sprintf(`fail to close DB for carry rows`))
			return
		}
	}
	...
	if carryRows != nil {
		...
		err := carryRows.Close()
		if err != nil {
			model.Log(``, model.LogLevelInfo, ``, model.SystemOther, fmt.Sprintf(`fail to close DB for carry rows`))
			return
		}
	}
